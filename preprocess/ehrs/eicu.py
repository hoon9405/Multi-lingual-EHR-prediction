...
class eICU(EHR):
        ...
        if (
            ...
            self.task_itemids = {
                ...
                "urine": { 
                    ...
                    "exclude": ["intakeoutputentryoffset", "intakeoutputid"],
                    "code": ["celllabel"],
                    "value": ["cellvaluenumeric"],
                    "itemid": ["Urine", "URINE CATHETER", "Urine"]
                },
                "AKI": {
                    "fname": "lab" + self.ext,
                    "timestamp": "labresultoffset",
    ...
    def make_compatible(self, icustays, spark):
        ...
        if "admissionweight" in icustays.columns:
            icustays = icustays.rename(columns={"admissionweight": "weight"})
        elif "admissionWeight" in icustays.columns:
            icustays = icustays.rename(columns={"admissionWeight": "weight"})
        else:
        ...
        if "gender" in icustays.columns:
            gender_series = icustays["gender"].astype(str).str.upper()
            male_mask = icustays["weight"].isna() & gender_series.isin(["M", "MALE"])
            female_mask = icustays["weight"].isna() & gender_series.isin(["F", "FEMALE"])
    ...
    def dialysis_filter(self, merge, timestamp, spark):
        ...
        numeric_cols = [
            ...
            "nettotal",
            "intakeoutputentryoffset",
            self.task_itemids["dialysis"]["timestamp"],
            "cellvaluenumeric",
        ]
        for col_name in numeric_cols:
            if col_name in io.columns:
                io = io.withColumn(col_name, F.col(col_name).cast("double"))

        io_dialysis = io.filter(
            F.col(dialysis_code).isNotNull() & (F.col(dialysis_code) != 0.0)
        )
        ...
        io_dialysis = io_dialysis.withColumn(
            "_DIALYSIS_TIME", F.col("_DIALYSIS_TIME").cast("double")
        )
    ...
    def clinical_task(self, cohorts, task, horizons, spark):
        ...
        # Treat table timestamps as ICU-relative offsets: convert to ADMIT-relative
        # by adding INTIME so later filters using (INTIME + window) work correctly.
        merge = merge.withColumn(
            timestamp,
            F.round(F.col(timestamp) + F.col("INTIME"))
        )
        ...
        for horizon in horizons:
            ...
            if task == "urine" and weight_lookup is not None:
                horizon_merge = horizon_merge.withColumn(
                    "_URINE_RAW",
                    F.col(value),
                )
            ...
            if task == "platelets":
                ...
                horizon_agg = horizon_agg.withColumn(
                    "rel_change",
                    (F.col("value") - F.col("baseline_creatinine")) / F.col("baseline_creatinine")
                )
                horizon_agg = horizon_agg.withColumn(
                    task_name,
