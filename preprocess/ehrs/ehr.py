...
from pyspark.sql.types import ArrayType, IntegerType, StructField, StructType, StringType
...
class EHR(object):
    def __init__(self, cfg):
        ...
        self.urine = cfg.urine
        self.AKI = cfg.AKI
        self.in_icu_mortality = cfg.in_icu_mortality
        
        self.dest = cfg.dest
        self.valid_percent = cfg.valid_percent
        self.seed = [int(s) for s in cfg.seed.replace(" ", "").split(",")]
        self.debug = cfg.debug
        self.verify_labeling = cfg.verify_labeling
    ...
    def build_cohorts(self, spark, cached=False):
        ...
        icustays = icustays.sample(frac=0.10, random_state=42) if self.debug else icustays
        icustays = self.make_compatible(icustays, spark)
        
        logger.info("Start building cohorts for {}".format(self.ehr_name))

        # filter out icustays with less than 24 hours of observation
    ...
    def prepare_tasks(self, cohorts, spark=None, cached=False):
        ...
        # prepare tasks with requried columns
        required_cols = [
            self.icustay_key,
            "INTIME",
            "ADMITTIME",
        ...
        # keep optional columns if present (e.g., weight for urine normalization)
        ...
        if self.in_icu_mortality:
            # INTIME < DEATHTIME < INTIME + LOS     
    ...
    def process_tables(self, cohorts, spark):
        ...
        for table in self.tables:
            ...
            if table["timeoffsetunit"] == "abs":
            ...
            elif table["timeoffsetunit"] == "ms" or "sec":
                div_unit = 1000 if table["timeoffsetunit"] == "ms" else 1
                events = events.join(
                ...
                events = events.withColumn(
                    "TIME",
                    F.col(timestamp_key) / div_unit / 60 - F.col("INTIME"),
                )
                events = events.drop(timestamp_key)
            ...
            if "code" in table:
                for code_idx in range(len(table["code"])):
                    ...
                    new_code_col_name = f"original_code_{code}"
                    events = events.withColumn(new_code_col_name, F.col(code))
                    original_code_cols.append(new_code_col_name)
            ...
            encoded_cols = {
                k: process_unit(k, self.column_type_id) for k in events.columns if k not in original_code_cols
            }
            ...
            schema = StructType(
                [
                    ...
                    StructField("original_texts", ArrayType(StringType()), False),
                    StructField("codes", ArrayType(StringType()), False),
                ]
            )

            def process_row(encoded_table_name, encoded_cols, original_code_cols):
                def _process_row(row):
                    ...
                    for col, val in row.items():
                        ...
                        if col in original_code_cols:
                            codes.append(str(val))
                            continue
                        
                        encoded_col = encoded_cols[col]
                        encoded_val = process_unit(str(val), self.value_type_id)
                        if (
                            len(input_ids)
                            + len(encoded_col[0])
                            + len(encoded_val[0])
                            ...
                            original_texts_parts.append(str(col))
                            original_texts_parts.append(str(val))
                        else:
                            break
                    
                    original_text = " ".join(original_texts_parts)
                    return input_ids, types, dpes, [original_text], codes
            ...
            events = (
                events.withColumn(
                    "tmp",
                    process_row(encoded_table_name, encoded_cols, original_code_cols)(
                ...
                .withColumn("original_texts", F.col("tmp.original_texts"))
                .withColumn("codes", F.col("tmp.codes"))
                .select(self.icustay_key, "TIME", "INPUTS", "TYPES", "DPES", "original_texts", "codes")
            )
    ...
    def make_input(self, cohorts, events, spark):
        ...
        def _make_input(events):
            ...
            original_texts = sum(df["original_texts"].tolist(), [])
            codes = sum(df["codes"].tolist(), [])
        ...
        for stay_id in tqdm(cohorts[self.icustay_key].values):
            ...
            if data.get("original_texts"):
                stay_g.create_dataset(
                    "original_texts",
                    data=[s.encode("utf-8") for s in data["original_texts"]],
            ...
            if data.get("codes"):
                stay_g.create_dataset(
                    "codes",
                    data=[s.encode("utf-8") for s in data["codes"]],
                    dtype=h5py.string_dtype(encoding="utf-8"),
                )
            ...
            for col in cohorts.columns:
                if col in ["ADMITTIME", "hi_start", "time"]:
                    continue
        ...
        selected_columns = [self.icustay_key] + [col for col in self.tasks if col in cohorts.columns]
    ...
    def _get_label_exclude_columns(self):
        exclude = {self.icustay_key, "INTIME", "ADMITTIME", "DEATHTIME", "LOS", "weight"}
        if self.hadm_key:
            exclude.add(self.hadm_key)
        if self.patient_key:
            exclude.add(self.patient_key)
        return exclude

    def _get_label_columns(self, df):
        exclude = self._get_label_exclude_columns()
        return [col for col in df.columns if col not in exclude]

    def _compute_label_stats(self, df):
        label_cols = self._get_label_columns(df)
        counts = {}
        all_labels = set()
        for col in label_cols:
            series = df[col]
            if not pd_types.is_numeric_dtype(series.dtype):
                continue
            vc = series.value_counts().sort_index()
            counts[col] = vc
            all_labels.update(vc.index.tolist())
        all_labels = sorted(all_labels)
        stats = pd.DataFrame(index=counts.keys(), columns=all_labels).fillna(0)
        for col, vc in counts.items():
            stats.loc[col, vc.index] = vc.values
        stats = stats.astype(int)
        return stats

    def _save_label_stats(self, stats_df):
        os.makedirs(self.dest, exist_ok=True)
        out_path = os.path.join(self.dest, f"label_stats_{self.ehr_name}.csv")
        stats_df.to_csv(out_path)
        logger.info("Saved label statistics to {}".format(out_path))

    def _fill_missing_label_values(self, df):
        for col in self._get_label_columns(df):
            series = df[col]
            if pd_types.is_categorical_dtype(series.dtype):
                if -1 not in series.cat.categories:
                    series = series.cat.add_categories([-1])
                df[col] = series.fillna(-1).astype(int)
            elif pd_types.is_numeric_dtype(series.dtype):
                df[col] = series.fillna(-1)
        return df
    ...
    def run_pipeline(self, spark) -> None:
        ...
        labeled_cohorts = self._fill_missing_label_values(labeled_cohorts)
        stats_df = self._compute_label_stats(labeled_cohorts)
        self._save_label_stats(stats_df)
        
        if self.verify_labeling:
            print("="*30)
            print(f"Label Statistics for {self.ehr_name}")
            print("="*30)
            for task, row in stats_df.iterrows():
                print(f"Task: {task}")
                print(row[row != 0].astype(int))
                print("-" * 20)
            return
