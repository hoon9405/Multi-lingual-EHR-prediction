...
class HIRID(EHR):
    ...
    def prepare_tasks(self, cohorts, spark, cached=False):
        ...
        # super().prepare_tasks 결과가 pandas DataFrame인 경우에만 Spark DF로 변환
        # (Spark가 bool/float 혼합 스키마를 merge할 때 DoubleType vs BooleanType 충돌을 피하기 위해 bool→float 캐스팅)
        if isinstance(labeled_cohorts, SparkDataFrame):
            labeled_cohorts_sdf = labeled_cohorts
        else:
            logger.info(
                "Cohort dtype snapshot before Spark conversion: %s",
                labeled_cohorts.dtypes.to_dict(),
            )
            bool_cols = [
                col for col in labeled_cohorts.columns
                if is_bool_dtype(labeled_cohorts[col])
            ]
            bool_like_cols = [
                col for col in labeled_cohorts.columns
                if col not in bool_cols
                and labeled_cohorts[col]
                .dropna()
                .apply(lambda x: isinstance(x, (bool, np.bool_)))
                .all()
            ]
            cols_to_cast = bool_cols + bool_like_cols
            if cols_to_cast:
                logger.info(
                    "Casting boolean cohort columns to float for Spark compatibility: %s",
                    cols_to_cast,
                )
                labeled_cohorts[cols_to_cast] = labeled_cohorts[cols_to_cast].astype(float)

            labeled_cohorts_sdf = spark.createDataFrame(labeled_cohorts)
        logger.info(
            "Spark schema after conversion: %s", labeled_cohorts_sdf.dtypes
        )
        ...
        logger.info(
            "Schema entering clinical tasks: %s", labeled_cohorts_sdf.dtypes
        )
        checkpoint_dir = os.path.join(self.cache_dir, "spark_checkpoints")
        os.makedirs(checkpoint_dir, exist_ok=True)
        spark.sparkContext.setCheckpointDir(checkpoint_dir)

        labeled_cohorts = labeled_cohorts_sdf
        for clinical_task in [
            ...
            if horizons:
                ...
                labeled_cohorts = labeled_cohorts.cache()
                labeled_cohorts = labeled_cohorts.checkpoint(eager=True)
                logger.info("Checkpointed cohorts after %s task", clinical_task)

        logger.info(
            "Schema after clinical tasks: %s", labeled_cohorts.dtypes
        )
        logger.info("Done preparing clinical task prediction for the given cohorts")
        labeled_cohorts_pdf = labeled_cohorts.toPandas()
        self.save_to_cache(labeled_cohorts_pdf, self.ehr_name + ".cohorts.labeled")
        return labeled_cohorts_pdf
    ...
    def make_compatible(self, icustays, spark):
        ...
        icustays["DEATHTIME"] = icustays["discharge_status"].map(
            lambda x: True if x == "dead" else np.nan,
        )
    ...
    def clinical_task(self, cohorts, task, horizons, spark):
        ...
        # Urine is recorded as ml/h (variableid=10020000). Convert to ml/kg/h using
        # a per-patient median weight (variableid=10000400) with 75kg fallback.
        if task == "urine":
            weight_table = spark.read.csv(os.path.join(self.data_dir, fname), header=True)
            weight_table = weight_table.select(self.icustay_key, code, value)
            weight_table = weight_table.filter(F.col(code) == 10000400).filter(
                F.col(value).isNotNull()
            )
            weight_table = weight_table.withColumn("_WEIGHT_KG", F.col(value).cast("float"))
            weight_table = weight_table.groupby(self.icustay_key).agg(
                F.percentile_approx("_WEIGHT_KG", 0.5).alias("_WEIGHT_KG")
            )
            merge = merge.join(weight_table, on=self.icustay_key, how="left")
            merge = merge.withColumn(
                "_WEIGHT_KG",
                F.coalesce(F.col("_WEIGHT_KG"), F.lit(75.0)),
            )
            merge = merge.withColumn(value, F.col(value) / F.col("_WEIGHT_KG"))
        ...
        for horizon in horizons:
            ...
            if task == "platelets":  # K/ul=G/l
            ...
            elif task == "creatinine":  # 값은 이미 umol/L
                horizon_agg = horizon_agg.withColumn(
                    task_name,
                ...
                horizon_agg = horizon_agg.withColumn(
                    ...
                    .when(
                        (F.col("rel_change") >= 0.3)
                        & (F.col("rel_change") < 1.5),
                        1,
                    )
                    .when(
                        (F.col("rel_change") >= 1.5)
                        & (F.col("rel_change") < 2.0),
                        2,
                    )
                    .when(
                        (F.col("rel_change") >= 2.0)
                        & (F.col("rel_change") < 3.0),
                        3,
                    )
